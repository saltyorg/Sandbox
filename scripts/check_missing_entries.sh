...
app_list=$(awk '/# Apps Start/{flag=1;next}/# Apps End/{flag=0}flag' "$PROJECT_ROOT/sandbox.yml" | awk '!/#/' | awk -F',|:' '/role/ {print $2}' | tr -d ' ' | grep -v -E "(settings|sanity_check|lgsm|plex-auto-languages)" | sort -u | tr '\n' ',' | sed 's/,$//')
